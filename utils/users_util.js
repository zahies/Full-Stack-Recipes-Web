...
async function getThreeLastRecipes(id){
    let name = await DB.execQuery(`SELECT Username From users WHERE UserID = '${id}'`);
    if(name.length===0){
        return null;
    }
    const three_last_id = await DB.execQuery(`SELECT top 3 RecipeID from dbo.user_recipes WHERE UserName = '${name[0].Username}' order by SeenDate desc`);
    if (three_last_id){
...
async function getFavoriteRecipes(id){
    ...
    const favorite_id = await DB.execQuery(`SELECT RecipeID from dbo.user_recipes WHERE UserName = '${name[0].Username}' and FavoriteFlag = 1 order by SeenDate desc`);
    if (favorite_id){
        return favorite_id;
    }else{
        return null;
...
async function getUserIDByname(name){
    const user_id = await DB.execQuery(`SELECT UserID FROM users WHERE Username = '${name}'`);
    return user_id;
}

async function updateRecipeFavorite(id,name,action){
    
    const ans = await DB.execQuery(`SELECT * FROM user_recipes WHERE Username = '${name}' and RecipeID = '${id}'`);
    if(ans.length===0){
        return false;
    }
    try{
        const ans1 = await DB.execQuery(`UPDATE user_recipes SET FavoriteFlag = '${action}' WHERE UserName = '${name}' and RecipeID = '${id}'`);
        return true;
    }catch(err){
        return false;
    }
...
async function getUserRecipes(id){
    ...
    const myRecipes = await DB.execQuery(`SELECT RecipeID FROM dbo.user_recipes WHERE UserName = '${name[0].Username}'`);
    let promises = [];
...
async function getInfoFullPersonalRecipe(id){
    try{
        const recipeinfo = await DB.execQuery(`SELECT * FROM recipes WHERE RecipeID ='${id.id}'`);
        const recIndgridients = await DB.execQuery(`SELECT * FROM recipe_ingredients WHERE RecipeID = '${id.id}'`)
...
async function getInfoFullFamilyRecipe(id){
    try{
        const recipeinfo = await DB.execQuery(`SELECT * FROM recipes WHERE RecipeID ='${id.id}'`);
        const recIndgridients = await DB.execQuery(`SELECT * FROM recipe_ingredients WHERE RecipeID = '${id.id}'`)
        const familyInfo = await DB.execQuery(`SELECT * FROM family_recipes WHERE RecipeID ='${id.id}'`)
...
const insertSeenDate = async function(id, userName){
    let exist1 = await DB.execQuery(`SELECT * FROM [dbo].[user_recipes] WHERE RecipeID =  '${id[0].RecipeID}'`);
    let favorite = await DB.execQuery(`SELECT FavoriteFlag FROM [dbo].[user_recipes] WHERE RecipeID = '${id[0].RecipeID}'`);
    return new Promise((res,rej) => {
        try{
            if (exist1.length > 0){
                DB.execQuery(`UPDATE dbo.user_recipes SET SeenDate = CURRENT_TIMESTAMP WHERE RecipeID = '${id[0].RecipeID}'`)
...
async function checkIfRecipeExist(id){
    var ID = parseInt(id);
        let exist = await DB.execQuery(`SELECT * FROM recipes WHERE API_ID =  '${ID}'`);
        if (exist.length > 0){
            return true;
        }else{
            return false; 
...
const getOurIDFromDB =  function(id) {
    return new Promise((res,rej) => {
        var ID = parseInt(id);
        let idE = DB.execQuery(`SELECT RecipeID FROM recipes WHERE API_ID =  '${ID}'`);
        if (idE){
            res(idE);
        }else{
...
const getOurIDFromDB2 =  function(id) {
    return new Promise((res,rej) => {
        let idE = DB.execQuery(`SELECT RecipeID FROM recipes WHERE RecipeID =  '${id.id}'`);
        if (idE){
            res(idE);
        }else{
            rej("something went wrong ! ");
...
async function isSeen(recID,user_id){
    let name = await DB.execQuery(`SELECT Username From users WHERE UserID = '${user_id}'`);
    const ans = await DB.execQuery(`SELECT * FROM user_recipes WHERE UserName = '${name[0].Username}' and RecipeID = '${recID}'`);
    if(ans.length===0){
        return false;
    }else{
        return true;
